...
struct shastate {
  unsigned long long h0,h1,h2,h3,h4;
#ifdef SHA_BIG_DATA  
  unsigned long long count;
#else  
  unsigned long count; //wxz unsigned short count;
#endif  
};
...
void SHA1(void) {
  unsigned char i;
  unsigned long long a,b,c,d,e,f,k,t;
  ...
  for (i=0; i<=79; i++) {
    if (i<=19) {
      f = d ^ (b & (c ^ d));
      k = (unsigned long long)0x5A827999;
    } else if (i<=39) {
      f = b ^ c ^ d;
      k = (unsigned long long)0x6ED9EBA1;
    } else if (i<=59) {
      f = (b & c) | (d & (b | c));
      k = (unsigned long long)0x8F1BBCDC;
    } else {
      f = b ^ c ^ d;
      k = (unsigned long long)0xCA62C1D6;
    }
...
// Hashes blocks of 64 bytes of data. //20
...
void SHA1Block(unsigned char* data, unsigned char len) {
  ...
  if (len<64) {
    ...
    // and do the padding
    message.w[len >> 2] &= (unsigned long long)init_FF << (((~len & 3)*8)+8);
    message.w[len >> 2] |= (unsigned long long)init_80 << ((~len & 3)*8);
    // there is space for a qword containing the size at the end
    // of the block
    if (len<=55) {
      message.w[15] = (unsigned long long)(shadigest.state.count) * 8;
    }
  }
...
{
 memcpy(temp, shadigest.data, size);
}  
